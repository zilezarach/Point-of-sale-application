  ...
  return (
    <div className="flex h-screen bg-gray-600">
      <div className="container mx-auto">
        ...
        <div className="mb-5">
          <input type="text" value={name} placeholder="Customer Name" onChange={(e) => setName(e.target.value)} className="rounded border border-rose-500 text-black w-1/2" required />
        </div>
        <div className="mb-5">
          <input type="text" value={phoneNumber} placeholder="Customer's Number" onChange={(e) => setPhoneNumber(e.target.value)} className="rounded border border-rose-500 text-black w-1/2" required />
        </div>
        {message && <p className="text-rose-600 font-bold">{message}</p>}
        <button onClick={handleSubmit} className="p-3 bg-rose-500 rounded hover:bg-indigo-700 mb-5">Register</button>
        ...
        <h2 className="font-bold text-2xl mt-7 mb-4">Send Notfications to Customers</h2>
        <div className="mt-5">
          <textarea value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Write the message here.." className="w-1/2 rounded border border-rose-500" />
        </div>
        <div className="mt-5 mb-5">
          <input type='checkbox' checked={sendToAll} onChange={() => setSendToAll(!sendToAll)} />
          <label>Send To All Customers</label>
        </div>
        <button className="p-3 rounded bg-rose-500 hover:bg-indigo-700 mb-5" onClick={handleSend}>Send Message</button>
      </div>
    </div>
